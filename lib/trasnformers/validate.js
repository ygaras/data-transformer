...
module.exports = function(input, options) {
  ...
  // TODO break it down to so that each type is validated in its own function and use librarires for parsing
  ...
  options.source.forEach((fieldName, index) => {
    if (options.type[index] === 'DateTime') {
      ...
      if (isNaN(result)) {
        ...
        // TODO This better be parsed using moment.js
      ...
      // TODO This should be changed to better parsing
      let result = parseFloat(input[fieldName].replace(',',''));
      output[fieldName] = result;
      if (isNaN(result)) {
        log.d(`value ${input[fieldName]} is not not a valid Float, entire record will be skipped`);
        passed = false;
      }
      log.d(`Parsing float value ${input[fieldName]} resulted in ${result}`);
